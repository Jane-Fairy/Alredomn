...
type (
	...
	User struct {
		...
		Nicknametwo string         `db:"nicknametwo"` // The nickname
...
func (m *defaultUserModel) Insert(ctx context.Context, data *User) (sql.Result, error) {
	...
	ret, err := m.conn.ExecCtx(ctx, query, data.Name, data.Password, data.Mobile, data.Gender, data.Nickname, data.Nicknametwo, data.Type)
	return ret, err
}

func (m *defaultUserModel) Update(ctx context.Context, newData *User) error {
	query := fmt.Sprintf("update %s set %s where `id` = ?", m.table, userRowsWithPlaceHolder)
	_, err := m.conn.ExecCtx(ctx, query, newData.Name, newData.Password, newData.Mobile, newData.Gender, newData.Nickname, newData.Nicknametwo, newData.Type, newData.Id)
	return err
}
